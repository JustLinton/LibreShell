...
pid_t exceProcess(int pIndex){
		...
		if(pid == 0) // 子进程执行代码段 
		{
			...
			if(prog[pIndex].tubeIn==1)
				{redirect_pipe(0,prog[pIndex-1].pipe);/*printf("%din:%d\n",pIndex, prog[pIndex-1].pipe[0]);*/}
			
			//检查并重定向stdout输出管道
			if(prog[pIndex].tubeOut==1)
				{redirect_pipe(1,prog[pIndex].pipe);/*printf("%dout:%d\n",pIndex,prog[pIndex].pipe[1]);*/}
